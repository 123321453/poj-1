...
Total Submissions: 9772		Accepted: 6227
Description
...
public class p1565 {
    public static void main(String[] args) {
        ...
        while(in.hasNext()) {
            ...
            int result = 0;
            int i,j;
            for(i = 0, j = (tmp.length); i < tmp.length ; i++,j--) {
                //System.out.println(tmp[i] +" " + ((int)Math.pow(2.0, (double)j)-1) );
                result += Integer.parseInt(Character.toString(tmp[i]))*((long)Math.pow(2.0, (double)j)-1);
            }
